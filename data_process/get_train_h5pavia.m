clear;close all;
%% settings
foldertest = './Pavia_SpanSlhs/train_17Isp';
folderlabel='./Pavia_Sres/label_17Hres';
size_input=32;
size_label=32;
stride =16;

savepath = 'train_splsres_pa.h5';
filepaths1 = dir(fullfile(foldertest,'*.mat'));
filepaths2 = dir(fullfile(folderlabel,'*.mat'));

%% initialization
data = zeros(size_input, size_input, 102, 1);
label = zeros(size_label, size_label, 102, 1);
...
for i=1:length(filepaths1)
    ...
    imgt=load(fullfile(foldertest,filepaths1(i).name));
    imgl=load(fullfile(folderlabel,filepaths2(i).name));
    
    image_train = imgt.S_panlhs;
    image_label = imgl.S_res;
